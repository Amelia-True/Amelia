...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  let stiker = false
       ...
       let g = typeof stick[1] !== "undefined" ? stick[1] : stickauth;
  try { 	
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (isUrl(args[0])) stiker = await sticker(false, args[0], global.packname, global.stickauth)
    ...
    if (stiker) conn.sendFile(m.chat, stiker, 'sticker.webp', '', m, null, purr)
